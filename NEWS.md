...
* Standard Error of Measurement: the ANOVA method now applies the general form of Emons (2023, Eq. 12) to merged score groups. Only Eq. 9 was used before, which holds when a score group contains a single sum score; in groups merged to reach the minimum group size the estimate kept the group's true-score variance and was inflated by `var(sum score in group) / (number of items - 1)`. The conditional SEM consequently drifted upward with the minimum group size and did not reduce to the alpha-based SEM when all scores formed one group. Score groups that were not merged are unaffected ([PR #354](https://github.com/jasp-stats/jaspReliability/pull/354)).
* Standard Error of Measurement: no confidence interval is reported where the conditional SEM is exactly zero, which happens when every respondent in a score group gave identical item responses, typically at the lowest or highest possible sum score. The previous zero-width interval implied perfect measurement precision. Affects the ANOVA, Thorndike, Lord and Keats methods; the conditional SEM values themselves are unchanged ([PR #354](https://github.com/jasp-stats/jaspReliability/pull/354)).
